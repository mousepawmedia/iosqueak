...
channel& channel::operator<<(const bool& rhs)
{
...
channel& channel::operator<<(const tril& rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
    switch(fmt.fmt_bool)
...
channel& channel::operator<<(const char& rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
channel& channel::operator<<(const std::string& rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
channel& channel::operator<<(const onechar& rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
channel& channel::operator<<(const onestring& rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
channel& channel::operator<<(const std::exception& rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
channel& channel::operator<<(const IOFormat& rhs)
{
    fmt = rhs;
    return *this;
}

channel& channel::operator<<(const IOCursor& rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
channel& channel::operator<<(const IOMemReadSize& rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
channel& channel::operator<<(const IOVrb& rhs)
{
    //Set the verbosity.
    vrb = rhs;
    //Revalidate parsing.
...
channel& channel::operator<<(const IOCat& rhs)
{
    //Set the category.
    cat = rhs;
    //Revalidate parsing.
...
channel& channel::operator<<(const IOCtrl& rhs)
{
...
channel& channel::resolve_pointer(const char* rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
channel& channel::resolve_pointer(const void* rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
template <typename T>
channel& channel::resolve_pointer(const T* rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
template channel& channel::resolve_pointer<bool>(const bool*);
template channel& channel::resolve_pointer<unsigned char>(const unsigned char*);
template channel& channel::resolve_pointer<int>(const int*);
template channel& channel::resolve_pointer<unsigned int>(const unsigned int*);
template channel& channel::resolve_pointer<short int>(const short int*);
template channel& channel::resolve_pointer<unsigned short int>(const unsigned short int*);
template channel& channel::resolve_pointer<long int>(const long int*);
template channel& channel::resolve_pointer<unsigned long int>(const unsigned long int*);
template channel& channel::resolve_pointer<long long int>(const long long int*);
template channel& channel::resolve_pointer<unsigned long long int>(const unsigned long long int*);
template channel& channel::resolve_pointer<float>(const float*);
template channel& channel::resolve_pointer<double>(const double*);
template channel& channel::resolve_pointer<long double>(const long double*);
template channel& channel::resolve_pointer<std::string>(const std::string*);
template channel& channel::resolve_pointer<onechar>(const onechar*);
template channel& channel::resolve_pointer<onestring>(const onestring*);

template <typename T>
channel& channel::resolve_integer(const T& rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
template channel& channel::resolve_integer<char>(const char&);
template channel& channel::resolve_integer<unsigned char>(const unsigned char&);
template channel& channel::resolve_integer<int>(const int&);
template channel& channel::resolve_integer<unsigned int>(const unsigned int&);
template channel& channel::resolve_integer<short int>(const short int&);
template channel& channel::resolve_integer<unsigned short int>(const unsigned short int&);
template channel& channel::resolve_integer<long int>(const long int&);
template channel& channel::resolve_integer<unsigned long int>(const unsigned long int&);
template channel& channel::resolve_integer<long long int>(const long long int&);
template channel& channel::resolve_integer<unsigned long long int>(const unsigned long long int&);

template <typename T>
channel& channel::resolve_float(const T& rhs)
{
    //If we cannot parse because of `shutup()` settings, abort.
    if(!can_parse()){return *this;}
...
template channel& channel::resolve_float<float>(const float&);
template channel& channel::resolve_float<double>(const double&);
template channel& channel::resolve_float<long double>(const long double&);

bool channel::apply_attributes()
{
    bool r = false;
...
void channel::inject(const char* str, bool recursive)
{
    //If we just applied attributes, push them now.
...
void channel::inject(const void* ptr, unsigned int len, bool dump)
{
    if(!dump)
    {
...
void channel::speakup(IOCat cat)
{
...
void channel::speakup(IOVrb vrb)
{
